...
mod tests {
    ...
    async fn build_query_handler(pool: PgPool) -> SearchRolesQueryHandler {
    ...
    #[sqlx::test]
    async fn test_search_roles(pool: PgPool) {
        let handler = build_query_handler(pool).await;
        let result = handler
            .query_cached(
                SearchRolesQuery::builder()
    ...
    #[sqlx::test]
    async fn test_search_roles_return_err_given_pool_is_closed(pool: PgPool) {
