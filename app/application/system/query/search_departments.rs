...
use crate::{
    ...
    system::dto::department::DepartmentDto,
};
use bon::Builder;
...
pub struct SearchDepartmentsQueryHandler {
    pool: PgPool,
    #[inject(|kvdb: Kvdb| CacheProvider::builder().key("system_search_departments:").ttl(Duration::from_secs(15 * 60)).kvdb(kvdb).build())]
...
impl QueryHandler for SearchDepartmentsQueryHandler {
    ...
    ) -> Result<PagingResult<DepartmentDto>, SystemError> {
