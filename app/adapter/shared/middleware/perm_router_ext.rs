...
use domain::auth::value_object::permission_group::{PermissionChecker, PermissionGroup};
...
macro_rules! perms {
    ($($p:expr),*) => {
        domain::auth::value_object::permission_group::PermissionGroup::new(std::collections::HashSet::from([$($p),*]))
    };
}
